...
public class LoginPresenter extends BasePresenter<LoginView> {
     ...
     */
    @Inject
    public LoginPresenter(@Nullable DataManager dataManager, @Nullable @ApplicationContext Context context, Activity activity) {
        super(context);
    ...
    private boolean isCredentialsValid(final String username, final String password) {
        ...
        if (username == null || username.matches("\\s*") || username.isEmpty()) {
            getMvpView().showUsernameError(getContext().getString(R.string.error_validation_blank,
                    getContext().getString(R.string.username)));
            credentialValid = false;
        } else if (username.length() < 5) {
            getMvpView().showUsernameError(getContext().getString(R.string.error_validation_minimum_chars
            ...
            getMvpView().showUsernameError(getContext().getString(
        ...
        if (password == null || password.isEmpty()) {
            getMvpView().showPasswordError(getContext().getString(R.string.error_validation_blank,
                    getContext().getString(R.string.password)));
            credentialValid = false;
        } else if (password.length() < 6) {
            getMvpView().showPasswordError(getContext().getString(R.string.error_validation_minimum_chars
     ...
     */
    private void saveAuthenticationTokenForSession(User user, String userName, long userID, String authToken) {
